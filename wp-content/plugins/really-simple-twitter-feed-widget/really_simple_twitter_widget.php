<?php
/*
Plugin Name: Really Simple Twitter Feed Widget Modded by HiQ
Plugin URI: http://www.whiletrue.it/
Description: Displays your public Twitter messages in the sidbar of your blog. Simply add your username and all your visitors can see your tweets! Search for "COREHACK" in really_simple_twitter_widget.php to find it.
Author: Original by WhileTrue, modified by janne.mikkonen@hiq.fi
Version: 2.5.2
      ...
      if (!isset($message['retweeted_status']) ) {
        $message['retweeted_status'] = array();
			...
			if($options['update']) {				
				...
				// COREHACK here - above $time is UTC, not localtime, so it needs to be adjusted when presented below.
				$h_time = ( ( abs( time() - $time) ) < 86400 ) ? sprintf( __('%s ago', 'rstw'), human_time_diff( $time )) : date($options['date_format'], $time + (current_time('timestamp') - time()));
